...
@section('title', 'Checklist Izin EWP')
@section('page-title', 'Checklist EWP: ' . $workPermit->nomor_pekerjaan)
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    {{-- Kolom 1: Checklist EWP --}}
    <div class="lg:col-span-1" id="checklist-ewp-container">
        <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100 mb-3 p-3 bg-slate-100 dark:bg-slate-800 rounded-t-lg border-b-2 border-yellow-600">
            Checklist Kerja Galian (EWP)
        </h3>
        <div class="space-y-4 p-4 bg-white dark:bg-slate-800 rounded-b-lg shadow-xl ring-1 ring-slate-200 dark:ring-slate-700">
            {{-- Loading state --}}
            <div id="loading-ewp">Memuat checklist...</div>
...
<script>
    // URL API
    const LOAD_URL = "{{ route('ewp-cek.index', $permitEwp->id) }}";
    const UPDATE_URL_BASE = "{{ url('ewp-cek') }}";
    ...
    async function loadChecklists() {
        try {
            ...
            renderChecklist('checklist-ewp-container', 'loading-ewp', result.data.ewp_items, canEditPemohon);
