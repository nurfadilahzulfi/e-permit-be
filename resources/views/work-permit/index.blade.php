...
<script>
    ...
    const toastContainer = document.getElementById('toast-container'); // Pastikan ini ada
            ...
            result.data.forEach(permit => {
                ...
                if (permit.status === 1) {
                    ...
                    if (permit.permit_cse) {
                        aksiContainer.appendChild(
                            createAksiButton(
                                `{{ url('cse-cek/view') }}/${permit.permit_cse.id}`, 
                                'Isi Checklist CSE', 
                                'text-purple-600', // Warna berbeda
                    ...
                    // (Nanti bisa tambahkan 'else if (permit.permit_hwg)' dll)
                }
                
                // 2. Tombol "Pekerjaan Selesai"
    ...
    function createAksiButton(href, title, colorClass, svgIcon, action = null) {
        ...
        button.title = title;
        // [PERBAIKAN] Class diganti agar lebih konsisten
