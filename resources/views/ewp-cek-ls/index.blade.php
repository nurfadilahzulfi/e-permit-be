...
@section('title', 'Manajemen Master EWP - E-Permit')
@section('page-title', 'Manajemen Master EWP')
...
<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 border-b pb-4 dark:border-slate-700">
    <h2 class="text-2xl font-extrabold text-slate-800 dark:text-slate-100 mb-3 sm:mb-0">
        Master Checklist EWP (Kerja Galian)
    </h2>
    <button id="addBtn"
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
            <thead class="bg-slate-50 dark:bg-slate-700">
                <tr>
                    <th class="py-3 px-4 text-left text-xs font-semibold text-slate-600 dark:text-slate-300 uppercase tracking-wider">Nama Checklist / Pertanyaan</th>
...
<div id="itemModal"
    ...
    <div id="modalBox"
        ...
        <div class="flex-shrink-0 px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-white dark:bg-slate-900 z-10 rounded-t-2xl">
            <h3 id="modalTitle" class="text-xl font-bold text-slate-800 dark:text-slate-100">Tambah Checklist EWP</h3>
        ...
        <div class="px-6 py-4 overflow-y-auto flex-1">
            <form id="itemForm">
                ...
                <div>
                    <label class="text-slate-700 dark:text-slate-200 text-sm font-medium">Nama Checklist / Pertanyaan</label>
                    <input id="nama" name="nama" type="text" required
                           class="w-full mt-1 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm dark:bg-slate-800 dark:text-slate-100 focus:ring-2 focus:ring-blue-500" placeholder="Contoh: Pengecekan utilitas bawah tanah sudah dilakukan">
                </div>
...
<script>
// =================================================================
// SCRIPT INI DI-SET UNTUK 'ewp-cek-ls'
// =================================================================
document.addEventListener('DOMContentLoaded', () => {
    
    // --- Variabel & Referensi [PERUBAHAN] ---
    const R_INDEX = '{{ route("ewp-cek-ls.index") }}';
    const R_STORE = '{{ route("ewp-cek-ls.store") }}';
    const R_PREFIX = 'ewp-cek-ls'; // prefix for update/delete/show
    ...
    function openAddModal() {
        ...
        document.getElementById('modalTitle').innerText = 'Tambah Checklist EWP Baru';
        showModal();
    }
    ...
    window.editItem = async (id) => {
        try {
            ...
            document.getElementById('modalTitle').innerText = 'Edit Checklist EWP';
            showModal();
        } catch (e) {
