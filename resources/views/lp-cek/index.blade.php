{{-- 
File ini menerima 2 variabel dari LpCekController:
- $permitLp: Model PermitLp (Anak)
- $workPermit: Model WorkPermit (Induk)
--}}
@extends('layouts.app')

@section('title', 'Checklist Izin LP')
@section('page-title', 'Checklist LP: ' . $workPermit->nomor_pekerjaan)
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
    ...
    {{-- Kolom 1: Checklist LP --}}
    <div class="lg:col-span-1" id="checklist-lp-container">
        <h3 class="text-lg font-bold text-slate-800 dark:text-slate-100 mb-3 p-3 bg-slate-100 dark:bg-slate-800 rounded-t-lg border-b-2 border-gray-600">
            Checklist Kerja Angkat (LP)
        </h3>
        <div class="space-y-4 p-4 bg-white dark:bg-slate-800 rounded-b-lg shadow-xl ring-1 ring-slate-200 dark:ring-slate-700">
            {{-- Loading state --}}
            <div id="loading-lp">Memuat checklist...</div>
...
<script>
    // URL API
    const LOAD_URL = "{{ route('lp-cek.index', $permitLp->id) }}";
    const UPDATE_URL_BASE = "{{ url('lp-cek') }}";
    ...
    async function loadChecklists() {
        try {
            ...
            renderChecklist('checklist-lp-container', 'loading-lp', result.data.lp_items, canEditPemohon);
